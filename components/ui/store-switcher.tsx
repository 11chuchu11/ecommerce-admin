...
import { Check, ChevronsUpDown, PlusCircle, Store as StoreICon, StoreIcon } from 'lucide-react'
		...
		<Popover open={open} onOpenChange={setOpen}>
			<PopoverTrigger asChild>
				<Button
                    ...
                    {currentStore?.label}
                    <ChevronsUpDown className="ml-auto h-4 w-4 shrink-0 opacityy-50" />
				</Button>
			</PopoverTrigger>
						...
						<CommandGroup heading="Stores">
							{formattedItems.map(store => (
								<CommandItem key={store.value} onSelect={() => onStoreSelected(store)} className="text-sm">
									...
									<Check className={cn('ml-4 h-4 w-4', currentStore?.value === store.value ? 'opacity-100' : 'opacity-0')} />{' '}
								</CommandItem>
							))}
						</CommandGroup>
                    </CommandList>
